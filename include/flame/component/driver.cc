...
void driver::on_loop() {
  ...
  try {
    if (_componentImpl) {
      if (_componentImpl->get_status() ==
          flame::component::dtype_status::STOPPED) {
        unsigned long long _rtime = _componentImpl->get_profile()
                                        ->raw()[def::PROFILE_RT_CYCLE_NS]
...
bool driver::load(
  ...
  try {
    ...
    create_component pfcreate =
        (create_component)dlsym(_component_handle, "create");
...
void driver::unload() {
  try {
    if (_componentImpl) {
      release_component pfrelease =
          (release_component)dlsym(_component_handle, "release");
